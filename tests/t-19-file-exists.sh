...
absent_registration="$($PMT_BIN watch --kind file-exists --host polaris --path /scratch/run/receipt --deadline +300)" || fail "remote ABSENT registration failed"
...
mock_ssh_script "1${tab}"
deadline_registration="$($PMT_BIN watch --kind file-exists --host polaris --path /scratch/run/never --deadline +1)" || fail "deadline ABSENT registration failed"

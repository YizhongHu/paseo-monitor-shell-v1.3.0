...
active_reg="$($PMT_BIN watch --script "$SANDBOX/probe" --reason 'cancel active watch' --terminal DONE --deliver "$SANDBOX/deliver" --deadline +300)" || fail "active registration failed"
...
: > "$MOCK_DIR/reports"
printf 'DONE terminal\n' > "$MOCK_DIR/done"
fired_reg="$($PMT_BIN watch --script "$SANDBOX/probe" --reason 'cancel fired watch' --terminal DONE --deliver "$SANDBOX/deliver" --deadline +300)" || fail "fired registration failed"
...
: > "$MOCK_DIR/reports"
all_one_reg="$($PMT_BIN watch --script "$SANDBOX/probe" --reason 'bulk cancellation one' --terminal DONE --deliver "$SANDBOX/deliver" --deadline +300)" || fail "bulk one registration failed"
all_one_id=$(printf '%s\n' "$all_one_reg" | sed -n 's/^watch \([^ ]*\) registered.*/\1/p')
all_two_reg="$($PMT_BIN watch --script "$SANDBOX/probe" --reason 'bulk cancellation two' --terminal DONE --deliver "$SANDBOX/deliver" --deadline +300)" || fail "bulk two registration failed"
...
failsafe_reg="$($PMT_BIN watch --script "$SANDBOX/probe" --reason 'rm failsafe cleanup' --terminal DONE --failsafe --max-runs 1 --expires-in 5m --deliver "$SANDBOX/deliver" --deadline +300)" || fail "rm failsafe registration failed"
...
: > "$MOCK_DIR/reports"
reap_reg="$($PMT_BIN watch --script "$SANDBOX/probe" --reason 'reap failsafe cleanup' --terminal DONE --failsafe --max-runs 1 --expires-in 5m --deliver "$SANDBOX/deliver" --deadline +300)" || fail "reap failsafe registration failed"
...
minimal_reg="$($PMT_BIN watch --script "$SANDBOX/minimal-probe" --reason 'minimal PATH cleanup' --terminal DONE --failsafe --max-runs 1 --expires-in 5m --deliver "$SANDBOX/deliver" --deadline +300)" || fail "minimal PATH registration failed"

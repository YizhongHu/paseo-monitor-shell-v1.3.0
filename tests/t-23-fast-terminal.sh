...
assert_eq "$(cat "$script_dir/fires")" 1 "script terminal report delivered at registration"
assert_eq "$(grep -c ' REPORT ' "$script_dir/log")" 1 "script terminal report count"
...
assert_eq "$(grep -c ' REPORT ' "$script_dir/log")" 1 "script terminal does not re-report"
...
assert_eq "$(cat "$slurm_dir/fires")" 1 "Slurm terminal report delivered at registration"
assert_eq "$(grep -c ' REPORT ' "$slurm_dir/log")" 1 "Slurm terminal report count"
printf '0\n' > "$slurm_dir/nextDue"
$PMT_BIN _sweep || fail "terminal Slurm follow-up sweep failed"
assert_eq "$(grep -c ' REPORT ' "$slurm_dir/log")" 1 "Slurm terminal does not re-report"
